...
```{r perc-atheist-us12}
us05 <- atheism %>% filter(nationality == "United States", year == 2005)
us12_responses <- us12 %>% summarise(cnt = n())
prop_us_ath_12 <- us12 %>% filter(response=='atheist') %>% summarise(cnt = n())/us12_responses
...
### Independence
  - This survey was performed in countries all around the world, and we can infer that sample collection methods were randomized. As such, we can be confident that the answers from people surveyed are independent of each other.
  - Additionally, because respondents were sampled without replacement, we want our sample size to be less than 10% of the world population, and 1,002 is well under that threshold.

### Sample Size/Skew | Success/Failure Condition
  - Because we are evaluating a proportion for one categorical variable with 2 levels (atheist vs not atheist), we need at least 10 observations from both sides (from successes and failures). Notice we have 50 atheists and 952 non-atheists, which fulfill this requirement.
  
Because we have fulfilled the independence condution, we can generalize our findings to the population of the entire US, and because we have fulfilled the success/failure condition, we can rely on Central Limit Theorem methods and the Normal Distribution for hypothesis testing.
...
**Response**
A margin of error comes with an estimate of some measure calculated from a sample that we want to use to infer a fact about the population at large. The margin of error is the amount *above* and *below* this estimate, creating a *window* or *interval* that we believe contains the true population measure.
...
india_12 <- atheism %>% filter(nationality == 'India', year == 2012) 
aust_12 <- atheism %>% filter(nationality == 'Australia', year == 2012) 

ci_aust_12 <- inference(y=response, data= aust_12, type = 'ci', statistic = 'proportion', success = 'atheist', method = 'theoretical')

ci_india_12 <- inference(y= response, data = india_12, type = 'ci', statistic = 'proportion', success = 'atheist', method = 'theoretical')
```
...
1. Independence: 
  ...
  - Total sample size from Spain is less than 10% of the entire population of Spain

2. Sample Size/Skew
  - Success/Failure Condition is met (>= 10 successes and >= 10 failures) for both groups (2005, 2012)


```{r spain-05-12}
# Creating a DF for Spain
spain <- atheism %>% filter(nationality == 'Spain')
# Counting responses for each year
cnt_reponses <- spain %>% group_by(year,response) %>% summarise(cnt=n())
# Inference testing comparing proportions between both years
...
1. Independence:
  - Respondents were randomly sampled
  - Total sampled respondents from each year was less than 10% of the US population
2. Sample Size/Skew:
  - Success/Failure condition was met (successes >= 10 & failures >= 10) for both groups (years)

```{r us-05-12}
us <- atheism %>% filter(nationality == 'United States')
us %>% group_by(year,response) %>% summarise(cnt=n())
inference(y=response,x=as.character(year),data=us,type='ht',statistic='proportion',success='atheist',null=0,method='theoretical',alternative='twosided')
