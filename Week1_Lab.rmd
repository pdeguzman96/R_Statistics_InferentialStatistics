...
```{r samp1-dist}
# Plotting a histogram of the area
ggplot(samp1,aes(area))+geom_histogram(binwidth = 250)
...
# Mean of the sample means is 1499. Median is 1497
sample_means50 %>% summarise(samp_means = mean(x_bar), samp_med = median(x_bar))
...
```{r price-sample}
ames %>% sample_n(50,replace=TRUE) %>% summarise(x_bar = mean(price))
...
```{r price-sampling}
sample_means50 <- ames %>% rep_sample_n(50,TRUE,5000) %>% summarise(x_bar=mean(price))
ggplot(sample_means50,aes(x_bar))+geom_histogram(bins=40)
...
sample_means150 <- ames %>% rep_sample_n(150,TRUE,5000) %>% summarise(x_bar=mean(price))
ggplot(sample_means150,aes(x_bar))+geom_histogram(bins=40)
```
...
sample_means15 <- ames %>% rep_sample_n(size=15,replace=TRUE,rep=2000) %>%
          summarise(x_bar=mean(price))
# Bimodal distribution
ggplot(sample_means15,aes(x_bar))+geom_histogram(bins=35)

# Mean price of the population
...
sample_means150 <- ames %>% rep_sample_n(size=150,replace=TRUE,reps=2000) %>%
          summarise(x_bar=mean(price))

ggplot(sample_means150,aes(x_bar))+geom_histogram(bins=35)
